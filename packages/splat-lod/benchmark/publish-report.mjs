...
const verdict = `**No clean win over PlayCanvas defaults yet.** Ours is slower on the two object scenes. On the shop, its median is only ${f(shopDefault.movingMs.median / shop.movingMs.median)}× faster, with worse image quality and a much worse moving p95: **${f(shop.movingMs.p95)} ms versus ${f(shopDefault.movingMs.p95)} ms**. The larger gains below are against the full-quality baseline, not default settings.\n\n`;
