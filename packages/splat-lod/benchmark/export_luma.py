"""Lossless parameter-layout conversion from SH3 Float32 PLY for luma.gl.
...
def export(source, destination):
    ...
    for start in range(0, len(src.pos), 262144):
        ...
        columns = [rows[:, :3], np.exp(rows[:, 52:55]), quaternion, colors, opacity,
                   rows[:, 6:51].reshape(-1, 3, 15).transpose(0, 2, 1)]
    ...
    (destination / 'manifest.json').write_text(json.dumps({
        'sourceSha256': src.fingerprint, 'count': len(src.pos), 'shDegree': 3,
        'layout': 'column-major positions3 scales3 WXYZ4 linearRGBA4 opacity1 basis-majorSH45 float32',
        'pages': pages
    }, indent=2) + '\n')
